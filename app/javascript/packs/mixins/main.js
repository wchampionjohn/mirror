...
export default {
  components: {
    PaginationVue, TableVue, PageSetterVue
  },
  data: {
    sources: [],
    enableEdit: true,
  ...
  methods: {
    ...
    selectedSources(){
      return this.sources.filter(
        source => this.selectedIds.indexOf(source.id) > -1
      );
    },
    selectorChange(selectorColumn) {
      var methodName = `${selectorColumn}SelectorChange`
      if (this[methodName]) {
        this[methodName]();
      }
    }
