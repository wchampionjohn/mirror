<template>
  <div class="row">
    <template v-for="queue in queues">
      <input type="hidden" name="platform_schedule[sessions_attributes][][id]" :value="queue.id">
      <input type="hidden" name="platform_schedule[sessions_attributes][][start_at]" :value="queue.period">
      <input type="hidden" name="platform_schedule[sessions_attributes][][queue]" :value="idsOf(queue)">
    </template>
    <div class="col-lg-6">
      <!--begin::Portlet-->
      ...
      <div class="m-portlet" id="m_portlet">
        ...
        <template v-for="(queue, index) in queues">
          <div class="m-portlet m-portlet--head-solid-bg m-portlet--head-sm" :class="(index % 2) == 0 ? 'm-portlet--primary' : 'm-portlet--success'">
              ...
              <div class="m-portlet__head-tools">
                <ul class="m-portlet__nav">
                  <li class="m-portlet__nav-item">
                    <a href="javascript:void(0)" @click="copy(index)" v-if="queue.rows.length > 0" data-toggle="m-tooltip" data-original-title="複製" class="m-portlet__nav-link m-portlet__nav-link--icon">
                      <i class="fa fa-copy"></i>
                    </a>
                    <a href="javascript:void(0)" @click="paste(index)"  data-toggle="m-tooltip" data-original-title="貼上" class="m-portlet__nav-link m-portlet__nav-link--icon">
                      <i class="fa fa-paste"></i>
                    </a>
                    <a href="javascript:void(0)" @click="switchCurrentQueue(index)" data-portlet-tool="toggle" class="m-portlet__nav-link m-portlet__nav-link--icon">
                      <i class="la" :class="currentQueue == queue ? 'la-angle-up' : 'la-angle-down'"></i>
                    </a>
                  </li>
                </ul>
              </div>
            ...
            <div class="m-portlet__body" v-show="queue == currentQueue">
...
export default {
  ...
  methods: {
    ...
    switchCurrentQueue(index) {
      var selectedQueue = this.queues.find((queue, _index) => {
        return index == _index;
      });

      if(selectedQueue == this.currentQueue) {
        this.currentQueue = {
          id: null,
          rows:[]
        };
      } else {
        this.currentQueue = selectedQueue;
      };
    },
    initializeQueues() {
      this.queues = [];
    ...
    paste(index) {
      ...
      target.rows = target.rows.concat(rows);
