module Render
  ...
  def serialize_data(data, options = {})
    unless data.nil?
      serialize_class = options[:serializer] || serialize_class(data)
      if data.respond_to?(:to_ary)
        data.map { |record| serialize_class.new(record, options).serializable_hash[:data][:attributes].as_json }
      else
        serialize_class.new(data, options).serializable_hash[:data][:attributes].as_json
      end
    end
  end
